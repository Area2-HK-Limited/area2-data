<template>
  <div>
    <Head>
      <title>立即訂閱 SecrexAI — 14日免費試用</title>
      <meta name="description" content="立即訂閱 SecrexAI，開始 14 日免費試用。支援月費/半年/年費計劃，公司 Email 登記需要信用卡。" />
    </Head>
    ...
    <section class="section" style="background: linear-gradient(135deg, #0A0E1A 0%, #0F1629 50%, #1A0A2E 100%); position: relative; overflow: hidden; padding-top: 4rem;">
      ...
      <div class="container" style="max-width: 800px; text-align: center; position: relative; z-index: 1;">
        <div class="badge badge-blue mb-2" style="display: inline-flex;">14 日免費試用</div>
    ...
    <section id="register-form" class="section" style="background: linear-gradient(135deg, #0F1629 0%, #0A0E1A 100%);">
      <div class="container" style="max-width: 640px;">
        <div class="text-center" style="margin-bottom: 2.5rem;">
          <div class="badge badge-blue mb-2" style="display: inline-flex;">選擇你的計劃</div>
          <h2 style="margin-top: 0.75rem;">立即開始你的 14 日免費試用</h2>
          <p style="color: #94A3B8; font-size: 0.9375rem; margin-top: 0.5rem;">試用期內可隨時取消，不收取任何費用</p>
        </div>
        ...
        <div v-else-if="submitted" class="card" style="text-align: center; padding: 3rem; border-color: rgba(34, 197, 94, 0.3);">
          ...
          <p style="color: #94A3B8; margin-bottom: 1.5rem;">
            {{ stripeStatus === 'success' 
              ? '恭喜！你已成功開始 14 日免費試用。請留意你嘅 Email 及 WhatsApp 確認。' 
              : '你已成功報名！' }}<br>
        ...
        <form v-else class="card" @submit.prevent="submitForm" novalidate style="padding: 2rem; border-color: rgba(59, 130, 246, 0.2);">
          <div class="grid-2" style="gap: 1rem; margin-bottom: 1.25rem;">
            ...
            <div class="form-group" style="margin-bottom: 0;">
              ...
              <div style="display: flex; gap: 0.5rem; align-items: flex-start;">
                <input v-model="form.whatsappCountry" type="text" class="form-input" placeholder="+852" maxlength="6" style="width: 80px; flex-shrink: 0;" />
                <input v-model="form.whatsapp" type="tel" class="form-input" placeholder="98765432" style="flex: 1;" required />
              </div>
          ...
          <div class="form-group" style="margin-top: 1.5rem;">
            ...
            <label class="form-label">選擇計劃<span style="color: #F87171;">*</span></label>
            ...
            <div class="plan-grid">
              ...
              >
                ...
                <p style="font-size: 0.75rem; color: #94A3B8; margin: 0;">5倍商業用量 · 進階功能 · 數據安全認證</p>
          ...
          <div class="form-group" style="margin-top: 1.5rem;">
            <label class="form-checkbox" style="align-items: flex-start;">
              <input type="checkbox" v-model="form.agreeTerms" style="margin-top: 3px;" />
            ...
            <div v-if="errors.agreeTerms" class="terms-error">{{ errors.agreeTerms }}</div>
          </div>
...
const errors = reactive({
  name: '', email: '', whatsapp: '', company: '', submit: '', agreeTerms: ''
})
...
const validate = () => {
  ...
  errors.company = ''
  errors.agreeTerms = ''
  if (!form.agreeTerms) { errors.agreeTerms = '請先閱讀並同意相關條款'; valid = false }
  ...
  if (!form.whatsapp || form.whatsapp.trim().length < 1) {
    errors.whatsapp = '請輸入 WhatsApp 號碼'
    valid = false
  }
...
const submitForm = async () => {
  ...
  try {
    ...
    // Auto-detect worker based on current deployment
    const host = typeof window !== 'undefined' ? window.location.hostname : ''
    const isPreview = host.includes('secrexai.pages.dev')
    const workerUrl = isPreview
      ? 'https://form-api-dev.area2hk-sub.workers.dev'
      : (config.public.workerUrl || 'https://form-api.area2hk-sub.workers.dev')
    ...
    const res = await fetch(workerUrl, {
      ...
      body: JSON.stringify({
        ...
        data: {
          ...
          name: form.name,
          email: form.email,
          phone: (form.whatsappCountry || '+852') + form.whatsapp,
...
const miniQA = [
  { q: '試用係真係免費嗎？', a: '係！14 日試用期內免費，需要填寫信用卡資料。試用期內可隨時取消，唔會扣費。' },
  { q: '試用期之後點算？', a: '14 日試用期結束後，如果你唔取消，會自動開始收費。你會喺到期前 3 日收到提醒。' },
  { q: '我可以用公司信用卡嗎？', a: '可以！我哋接受所有主要信用卡（包括公司卡）。' },
  { q: '隨時可以取消嗎？', a: '可以！喺試用期內或試用期後，隨時可以喺 WhatsApp 向我們發送「取消」來取消訂閱。' },
...
/* Submit button disabled state (agreeTerms unchecked) */
