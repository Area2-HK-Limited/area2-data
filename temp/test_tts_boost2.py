...
# Test different language_boost formats
for lang_boost in ["Chinese,Yue", "Cantonese", "yue", "Chinese"]:
    payload = {
        "model": model,
        "text": text,
        "stream": False,
    ...
    try:
        ...
        if status == 0:
            ...
            with open(output.replace(".mp3", f"_{lang_boost.replace(',', '_')}.mp3"), "wb") as f:
                f.write(audio_bytes)
            print(f'"{lang_boost}" -> OK: {len(audio_bytes)} bytes')
        else:
            print(f'"{lang_boost}" -> ERROR {status}: {base_resp.get("status_msg", "unknown")}')
    except Exception as e:
        print(f'"{lang_boost}" -> Exception: {e}')
