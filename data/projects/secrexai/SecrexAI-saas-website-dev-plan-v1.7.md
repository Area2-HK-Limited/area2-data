...
> **所有圖片由 `skills/image-generation`（Nano Banana Pro via Google AI Studio API）生成**，唔用 stock photo，唔用 DALL-E。
> 圖片總數：約 **45 張**（行業插圖 15 張 + Persona 頭像 12 張 + 其他頁面用圖）
...
### 6.5 圖片生成 Build Pipeline（Phase 0 必須完成）

> ⚠️ **網站 build 前必須先生成所有 AI 圖片！唔可以用 placeholder emoji 或文字代替上線。**

**生成工具：** `skills/image-generation`（Nano Banana Pro via Google AI Studio API）

**每頁圖片 Checklist（約 45 張）：**

| 頁面 | 圖片 | 數量 | 尺寸 | 位置 | Prompt 方向 |
|------|------|------|------|------|------------|
| 首頁 Hero | 主圖 | 1 | 800×600 | Hero 右側 | 香港辦公室老闆用 WhatsApp 同 AI 助手對話，暗色背景，Pixar 3D |
| 首頁行業 | 行業插圖 | 15 | 400×300 | 行業 Tab 面板 | 各行業場景扁平設計，暗色背景，帶微光邊框 |
| 首頁 Persona | 人物頭像 | 12 | 100×100 | Persona 卡片（圓形） | 多元香港面孔，Pixar 3D，暗色背景 |
| 安全頁 | 數據中心圖 | 1 | 600×400 | 安全承諾 Section | 現代伺服器，藍色 LED，亞太數據中心，暗色調 |
| AI 教練 | 客服場景圖 | 1 | 600×400 | 服務介紹旁 | 友善技術人員，WhatsApp 協助客人，溫暖插圖 |
| 關於 | 公司合作圖 | 1 | 600×400 | Hostlink×Area2 旁 | 兩間公司合作，香港城市背景 |
| Early Access | Hero 裝飾圖 | 1 | 600×400 | Hero Section | 3D WhatsApp + AI 助手小人，暗色背景 |
| Blog 封面 | 文章封面 | 5 | 800×400 | Blog 列表 | 按文章主題生成 |
| OG Image | Social 分享圖 | 1 | 1200×630 | Meta tags | Logo + 標語，深色背景，品牌感 |
| Background | 紋理素材 | 1-2 | Tileable | Section 背景 | 深色微妙粒子/光點紋理 |
| Pricing | 裝飾 | 1 | 600×300 | 定價頁 Hero | 比較圖表風格，暗色 |
| Contact | 聯絡場景 | 1 | 600×400 | Contact 頁 | 友善溝通場景 |
| **合計** | | **~45** | | | |
...
Phase 0 開始
    ↓
Step 1: 生成所有 AI 圖片（~45 張）
    工具：skills/image-generation (Nano Banana Pro via Google AI Studio API)
    輸出：public/images/generated/{section}/
    命名：{section}-{description}.webp（例：hero-whatsapp-ai-assistant.webp）
    ↓
Step 2: 圖片後處理
    - WebP 壓縮（ImageMagick / sharp）
    - 響應式尺寸（1x / 2x @2x suffix）
    - Alt text JSON mapping（public/images/alt-texts.json）
    ↓
Step 3: 嵌入網站代碼
    - 用 <NuxtImg> 組件引用（Nuxt Image module）
    - loading="lazy"（首屏圖片除外）
    - 每張都要 alt text（SEO + 無障礙）
    ↓
Step 4: 上線前驗證
    - 檢查零 broken image（404）
    - 檢查 WebP fallback
    - Lighthouse 圖片評分 > 90
    ↓
Phase 1 開發繼續...
...
│   ├── pricing/       # 定價頁
│   ├── contact/       # 聯絡頁
│   └── shared/        # OG image、背景紋理
├── alt-texts.json     # 所有圖片 alt text mapping
└── logo/              # Logo 相關（手工設計）
```

**開發規則（寫入 AGENTS.md / Coding Guidelines）：**

1. ❌ 絕對唔好用 placeholder emoji / Heroicons 代替實際圖片上線
2. ❌ 唔好用 `<img src="placeholder.png">`
3. ✅ 每張圖必須有 `alt` text
4. ✅ 格式優先 WebP，JPG fallback
5. ✅ 首屏圖片 eager loading，其餘 lazy
6. ✅ Dark theme 適配：所有圖片需暗色背景或透明底
...
- **所有頁面圖片正確顯示，零 broken image（零 404）**
...
- [ ] **⭐ 生成所有 AI 圖片（約 45 張，Nano Banana Pro）— 網站 build 前必須完成，唔可以用 placeholder！**（`skills/image-generation`，行業 15 張 + Persona 12 張 + 其他頁面圖，見 Section 6.5）
...
### 2026-03-23：v1.6a → v1.7（圖片 Build Pipeline 強制規範）

#### v1.6a → v1.7（2026-03-23）
- **圖片 Build Pipeline** — 新增 Section 6.5，明確「build 前必須生成所有 AI 圖片」流程
- 每頁圖片 Checklist（~45 張，含尺寸/位置/Prompt 方向）
- 圖片存放結構規範（`public/images/generated/{section}/`）
- 4 步 Pipeline：生成 → 後處理 → 嵌入 → 驗證
- 6 條開發規則（禁 placeholder、必須 alt text、WebP 優先等）
- Phase 0 Checklist 強化圖片要求（⭐ 必須完成，唔可以用 placeholder）
- Phase 1 驗收標準加入：「所有頁面圖片正確顯示，零 broken image」
- Section 6 圖片需求清單：總數更新至約 45 張（行業 15 張 + Persona 12 張 + 其他）
- 版本號：v1.6a → v1.7
