<template>
  <main>
    <Head>
      <title>常見問題 FAQ | PersonAI — 香港 WhatsApp AI 助手</title>
      <meta name="description" content="PersonAI 常見問題解答。了解功能、定價、安全、技術支援等所有你想知道的答案。" />
    </Head>

    <section class="bg-gradient-to-br from-blue-50 to-white py-16">
      <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-12">
          <h1 class="text-4xl font-black text-gray-900 mb-4">常見問題</h1>
          <p class="text-gray-500 text-lg">找唔到答案？<a href="https://wa.me/85296456787" target="_blank" class="text-blue-600 hover:underline">WhatsApp 直接問我哋</a></p>
        </div>

        <div v-for="(category, ci) in faqCategories" :key="ci" class="mb-10">
          <h2 class="text-xl font-black text-gray-900 mb-4 flex items-center gap-2">
            <span>{{ category.emoji }}</span>{{ category.title }}
          </h2>
          <div class="space-y-3">
            <div v-for="(faq, fi) in category.faqs" :key="fi" class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
              <button @click="toggle(ci, fi)" class="w-full px-6 py-4 text-left flex items-center justify-between gap-4 hover:bg-gray-50 transition-colors">
                <span class="font-semibold text-gray-900 text-sm">{{ faq.q }}</span>
                <span class="text-gray-400 flex-shrink-0 text-xs">{{ isOpen(ci, fi) ? '▲' : '▼' }}</span>
              </button>
              <div v-if="isOpen(ci, fi)" class="px-6 pb-4 text-gray-600 text-sm leading-relaxed">{{ faq.a }}</div>
            </div>
          </div>
        </div>

        <div class="text-center bg-blue-600 rounded-2xl p-8 text-white">
          <h3 class="font-black text-xl mb-3">仍然有疑問？</h3>
          <p class="text-blue-200 mb-6">我哋隨時以 WhatsApp 廣東話回答你的問題</p>
          <a href="https://wa.me/85296456787?text=你好！我有關於 PersonAI 的問題想查詢" target="_blank" class="inline-flex items-center gap-2 bg-green-400 text-white px-8 py-3 rounded-xl font-bold hover:bg-green-500 transition-colors">
            💬 WhatsApp 即時咨詢
          </a>
        </div>
      </div>
...
const toggle = (ci: number, fi: number) => {
  const key = `${ci}-${fi}`
  if (openItems.value.has(key)) openItems.value.delete(key)
  else openItems.value.add(key)
}

const isOpen = (ci: number, fi: number) => openItems.value.has(`${ci}-${fi}`)
