<template>
  <main>
    <!-- HERO -->
    <section class="section-hero grid-texture pt-16 pb-24 relative">
      <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div class="text-center lg:text-left">
            <div class="badge mb-6 inline-flex">🇭🇰 香港 SME 專屬 · 唔需要 VPN</div>
            <h1 class="text-4xl md:text-5xl lg:text-6xl font-black mb-6 leading-tight">
              住喺你<br><span class="gradient-text">WhatsApp</span> 的<br>24 小時全能 AI 助手
            </h1>
            <p class="text-lg text-[#94A3B8] mb-8 leading-relaxed">
              廣東話原生 · 永久記憶 · 60+ 技能 · 數據存亞太區（日本）<br>
              唔訓練模型 · PDPO 合規 · 真人 WhatsApp 技術支援
            </p>
            <div class="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start mb-8">
              <NuxtLink to="/early-access" class="btn-primary text-center">🚀 開始 14 日免費試用</NuxtLink>
              <NuxtLink to="/features" class="btn-secondary text-center">📖 了解功能</NuxtLink>
            </div>
            <div class="flex flex-wrap gap-2 justify-center lg:justify-start">
              <span class="trust-badge">🏢 Hostlink × Area2</span>
              <span class="trust-badge">🔒 PDPO 合規</span>
              <span class="trust-badge">🇯🇵 數據存日本</span>
              <span class="trust-badge">✅ ISO 27001</span>
            </div>
          </div>
          <div class="flex justify-center">
            <div class="demo-phone w-full max-w-sm animate-float">
              <div class="demo-phone-header">
                <div class="w-9 h-9 rounded-full bg-white/20 flex items-center justify-center text-xl">🤖</div>
                <div><div class="font-semibold text-sm text-white">PersonAI</div><div class="text-xs text-green-200">● 在線</div></div>
              </div>
              <div class="demo-phone-body">
                <div class="chat-bubble-user text-sm">幫我搜尋今日香港財經新聞，重點係 AI 科技行業 🔍</div>
                <div class="chat-bubble-ai text-sm">
                  <div class="font-semibold text-blue-400 mb-1">🤖 PersonAI</div>
                  好嘅！我幫你整理：<br><br>📊 <strong>今日重點</strong><br>1. 阿里巴巴 AI 業務季度增長 28%<br>2. 香港金管局發布 AI 監管指引<br>3. 本地 FinTech 融資創歷史新高<br><br>需要我深入分析某個話題嗎？
                </div>
                <div class="chat-bubble-user text-sm">同埋幫我提醒 3pm 同客戶開會</div>
                <div class="chat-bubble-ai text-sm">
                  <div class="font-semibold text-blue-400 mb-1">🤖 PersonAI</div>
                  ✅ 已設定！我會喺 2:45pm 按排程發你提醒，順帶整理議程摘要。
                </div>
              </div>
            </div>
          </div>
    ...
    <section class="section-darker py-20">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-12">
          <h2 class="text-3xl md:text-4xl font-black mb-4">你係咪都有呢幾個問題？ 🤔</h2>
          <p class="text-[#94A3B8] text-lg">PersonAI 全部幫你解決</p>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <div v-for="pain in painPoints" :key="pain.id" class="feature-card text-center">
            <div class="text-3xl mb-4">{{ pain.emoji }}</div>
            <h3 class="font-bold text-white mb-2 text-sm">{{ pain.title }}</h3>
            <p class="text-[#94A3B8] text-xs leading-relaxed">{{ pain.desc }}</p>
          </div>
        </div>
        <div class="text-center mt-10">
          <span class="gradient-text font-black text-lg">↓ PersonAI 全部解決 ↓</span>
        </div>
      </div>
    </section>
    ...
    <section class="section-blue py-20">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-10">
          <h2 class="text-3xl md:text-4xl font-black mb-4">PersonAI 點幫你？揀你嘅行業 👇</h2>
          <p class="text-[#94A3B8]">睇吓 PersonAI 點為你解決痛點</p>
        </div>
        <div class="flex gap-2 overflow-x-auto pb-4 mb-8">
          <button v-for="ind in industries" :key="ind.id" @click="activeIndustry = ind.id"
            class="industry-tab flex-shrink-0" :class="{ active: activeIndustry === ind.id }">
            {{ ind.emoji }} {{ ind.name }}
          </button>
        </div>
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8" v-if="currentIndustry">
          <div class="glass-card p-6">
            <h3 class="text-lg font-bold text-red-400 mb-4 flex items-center gap-2">📊 你的痛點</h3>
            <ul class="space-y-3">
              <li v-for="pain in currentIndustry.pains" :key="pain" class="flex items-start gap-3">
                <span class="text-red-400 mt-0.5">✗</span>
                <span class="text-[#E2E8F0] text-sm">{{ pain }}</span>
              </li>
            </ul>
          </div>
          <div class="glass-card p-6">
            <h3 class="text-lg font-bold text-green-400 mb-4 flex items-center gap-2">✨ PersonAI 如何解決</h3>
            <ul class="space-y-3">
              <li v-for="sol in currentIndustry.solutions" :key="sol" class="flex items-start gap-3">
                <span class="text-green-400 mt-0.5">✓</span>
                <span class="text-[#E2E8F0] text-sm">{{ sol }}</span>
              </li>
            </ul>
          </div>
        </div>
        <div class="text-center mt-8">
          <NuxtLink to="/early-access" class="btn-primary">💬 體驗 PersonAI 幫你解決 →</NuxtLink>
        </div>
      </div>
    </section>
    ...
    <section class="section-darker py-20">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-10">
          <h2 class="text-3xl md:text-4xl font-black mb-4">體驗一下，PersonAI 點幫你？ 🎭</h2>
          <p class="text-[#94A3B8]">選擇你嘅身份，睇模擬對話</p>
        </div>
        <div class="flex gap-2 overflow-x-auto pb-4 mb-8 justify-center flex-wrap">
          <button v-for="role in roles" :key="role.id" @click="switchRole(role.id)"
            class="role-btn" :class="{ active: activeRole === role.id }">
        ...
        <div class="max-w-md mx-auto">
          <div class="demo-phone">
            ...
            <div class="demo-phone-body" style="min-height:300px">
              <template v-for="(msg, idx) in visibleMessages" :key="idx">
                <div :class="msg.type === 'user' ? 'chat-bubble-user' : 'chat-bubble-ai'" class="text-sm">
                  <span v-if="msg.type === 'ai'" class="font-semibold text-blue-400 mb-1 block">🤖 PersonAI</span>
                  <span v-html="msg.text"></span>
                </div>
              </template>
              <div v-if="isTyping" class="chat-bubble-ai">
                <div class="typing-dots flex gap-1 items-center py-1"><span></span><span></span><span></span></div>
              </div>
            </div>
          </div>
          <div class="text-center mt-6 flex gap-4 justify-center">
            <button @click="restartDemo" class="btn-secondary text-sm px-6 py-3">🔄 再睇一次</button>
            <NuxtLink to="/early-access" class="btn-whatsapp text-sm px-6 py-3">🚀 親身試用 →</NuxtLink>
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="section-blue py-20">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-10">
          <h2 class="text-3xl md:text-4xl font-black mb-4">真實工作情景，睇吓 PersonAI 點幫你 ⚡</h2>
          <p class="text-[#94A3B8]">一個指令，搞掂多個步驟</p>
        </div>
        <div class="flex gap-2 overflow-x-auto pb-4 mb-8 justify-center flex-wrap">
          <button v-for="scene in scenes" :key="scene.id" @click="activeScene = scene.id"
            class="role-btn text-sm" :class="{ active: activeScene === scene.id }">
            {{ scene.emoji }} {{ scene.title }}
          </button>
        </div>
        <div class="glass-card p-8 max-w-3xl mx-auto" v-if="currentScene">
          <div class="flex items-center gap-3 mb-6">
            <span class="text-3xl">{{ currentScene.emoji }}</span>
            <div>
              <h3 class="text-xl font-bold text-white">{{ currentScene.title }}</h3>
              <p class="text-[#94A3B8] text-sm">{{ currentScene.context }}</p>
            </div>
          </div>
          <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
            <div v-for="(step, idx) in currentScene.steps" :key="idx" class="text-center">
              <div class="w-12 h-12 rounded-full bg-[#1E293B] border border-[#334155] flex items-center justify-center text-xl mx-auto mb-2">{{ step.emoji }}</div>
              <div class="text-xs font-bold text-[#94A3B8] mb-1">{{ step.skill }}</div>
              <div class="text-xs text-[#64748B]">{{ step.action }}</div>
            </div>
          </div>
          <div class="text-center">
            <span class="badge badge-green">⏱️ {{ currentScene.timeSaved }}</span>
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="section-darker py-20">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-12">
          <h2 class="text-3xl md:text-4xl font-black mb-4">適合你嗎？ 👤</h2>
          <p class="text-[#94A3B8]">如果你有類似嘅問題，PersonAI 幫到你</p>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          <div v-for="p in personas" :key="p.id" class="feature-card">
            <div class="flex items-start gap-4 mb-4">
              <div class="text-4xl flex-shrink-0">{{ p.emoji }}</div>
              <div>
                <h3 class="font-bold text-white text-sm">{{ p.name }}</h3>
                <p class="text-[#94A3B8] text-xs">{{ p.role }}</p>
              </div>
            </div>
            <div class="bg-[#0F172A] rounded-lg p-3 mb-3">
              <div class="text-red-400 text-xs font-bold mb-1">😣 痛點</div>
              <p class="text-[#94A3B8] text-xs leading-relaxed">{{ p.pain }}</p>
            </div>
            <div class="bg-[#0F172A] rounded-lg p-3">
              <div class="text-green-400 text-xs font-bold mb-1">✅ PersonAI</div>
              <p class="text-[#94A3B8] text-xs leading-relaxed">{{ p.solution }}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <section class="py-20" style="background: linear-gradient(135deg, #1E3A5F 0%, #0F172A 100%);">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-12">
          <h2 class="text-3xl md:text-4xl font-black mb-4 text-white">你的業務資料，只屬於你 🔒</h2>
          <p class="text-blue-200 text-lg">SME 最擔心的 4 個問題，PersonAI 一一回應</p>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          <div v-for="p in securityPromises" :key="p.id" class="security-card">
            <div class="text-3xl mb-4">{{ p.emoji }}</div>
            <h3 class="font-bold text-white text-lg mb-3">{{ p.title }}</h3>
            <p class="text-[#94A3B8] text-sm leading-relaxed">{{ p.desc }}</p>
          </div>
        </div>
        <div class="text-center mb-8">
          <div class="inline-flex flex-wrap gap-3 justify-center">
            <span v-for="cert in certifications" :key="cert" class="cert-badge">{{ cert }}</span>
          </div>
        </div>
        <div class="text-center">
          <NuxtLink to="/security" class="btn-secondary border-blue-500 text-blue-400 hover:bg-blue-500/10">📖 了解更多安全承諾 →</NuxtLink>
        </div>
      </div>
    </section>
    ...
    <section class="section-darker py-20">
      <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div>
            <span class="badge mb-4">🎓 AI 教練服務</span>
            <h2 class="text-3xl md:text-4xl font-black mb-4 text-white">唔識用？有人教！</h2>
            <p class="text-[#94A3B8] text-lg mb-6 leading-relaxed">真人技術團隊，WhatsApp 廣東話全程支援。唔係冷冰冰嘅 Chatbot，係真係幫到你嘅 AI 教練。</p>
            <ul class="space-y-3 mb-8">
              <li v-for="item in coachItems" :key="item" class="flex items-center gap-3 text-[#E2E8F0] text-sm">
                <span class="text-green-400 font-bold">✓</span> {{ item }}
              </li>
            </ul>
            <div class="text-2xl font-black mb-2 gradient-text">HK$1,500/月</div>
            <p class="text-[#64748B] text-sm mb-6">月付，隨時取消 · 可與任何方案配合</p>
            <NuxtLink to="/ai-coach" class="btn-primary">了解 AI 教練服務 →</NuxtLink>
          </div>
          <div class="text-center">
            <div class="glass-card p-8 inline-block">
              <div class="text-6xl mb-4">🎓</div>
              <h3 class="text-xl font-bold text-white mb-2">真人 WhatsApp 支援</h3>
              <p class="text-[#94A3B8] text-sm">唔係 AI Chatbot，係真係幫到你嘅教練</p>
              <div class="mt-6 text-sm text-[#64748B]">適合：唔想自己摸索，想快速上手嘅用戶</div>
            </div>
          </div>
        </div>
      </div>
    ...
    <section class="section-blue py-20">
      <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-12">
          <h2 class="text-3xl md:text-4xl font-black mb-4">揀啱你嘅方案 💰</h2>
          <p class="text-[#94A3B8]">所有方案包含 60+ 技能 + 廣東話優化 + 永久記憶</p>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div v-for="plan in pricingPlans" :key="plan.id" class="pricing-card" :class="{ featured: plan.featured }">
            <div class="text-center mb-6">
              <div class="text-sm font-bold text-[#94A3B8] mb-1">{{ plan.target }}</div>
              <div class="text-3xl font-black mb-1" :class="plan.featured ? 'gradient-text' : 'text-white'">{{ plan.name }}</div>
              <div class="text-3xl font-black text-[#F97316]">{{ plan.price }}</div>
              <div class="text-[#64748B] text-sm">{{ plan.period }}</div>
            </div>
            <ul class="space-y-2 mb-6">
              <li v-for="feat in plan.features" :key="feat" class="flex items-center gap-2 text-[#94A3B8] text-sm">
                <span class="text-green-400 text-xs">✓</span> {{ feat }}
              </li>
            </ul>
            <NuxtLink v-if="plan.id !== 'enterprise'" to="/early-access"
            ...
            <a v-else href="https://wa.me/85296456787?text=你好，我想查詢 Enterprise 方案" target="_blank"
              class="block w-full text-center py-3 rounded-xl font-bold text-sm border border-[#334155] text-[#94A3B8] hover:border-[#3B82F6] transition-all">
              📱 WhatsApp 查詢
            </a>
          </div>
        </div>
        <div class="text-center">
          <NuxtLink to="/pricing" class="btn-secondary">睇完整比較表 →</NuxtLink>
        </div>
      </div>
    </section>
    ...
    <section class="section-darker py-20">
      <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-10">
          <h2 class="text-3xl md:text-4xl font-black mb-4">常見問題 ❓</h2>
        </div>
        <div class="space-y-3">
          <div v-for="(faq, idx) in faqs" :key="idx" class="faq-item">
            <div class="faq-question" @click="toggleFaq(idx)">
              <span>{{ faq.q }}</span>
              <svg class="w-5 h-5 flex-shrink-0 transition-transform" :class="{ 'rotate-180': openFaq === idx }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
              </svg>
            </div>
            <div class="faq-answer" :class="{ open: openFaq === idx }">{{ faq.a }}</div>
          </div>
        </div>
        <div class="text-center mt-8">
          <NuxtLink to="/faq" class="btn-secondary">查看全部 FAQ →</NuxtLink>
        </div>
      </div>
    </section>
    ...
    <section class="section-hero py-20">
      <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <h2 class="text-3xl md:text-5xl font-black mb-6 text-white">
          14 日免費試用<br><span class="gradient-text">唔需要信用卡</span>
        </h2>
        <p class="text-xl text-[#94A3B8] mb-8">試用期內可以隨時取消，唔會自動扣費</p>
        <div class="flex flex-col sm:flex-row gap-4 justify-center mb-6">
          <NuxtLink to="/early-access" class="btn-primary text-lg px-10 py-4">🚀 立即開始免費試用</NuxtLink>
          <a href="https://wa.me/85296456787?text=你好，我想了解 PersonAI" target="_blank" class="btn-whatsapp text-lg px-10 py-4">💬 WhatsApp 查詢</a>
        </div>
        <p class="text-[#64748B] text-sm">有問題？直接 WhatsApp 聯絡我哋 📱 +852 9645 6787</p>
      </div>
    </section>
  </main>
</template>
...
const visibleMessages = ref<Array<{type:string,text:string}>>([])
...
const restartDemo = () => {
  ...
  const msgs = roles.find(r => r.id === activeRole.value)?.messages || []
  playMessages(msgs, 0)
}

const playMessages = (msgs: Array<{type:string,text:string}>, idx: number) => {
  ...
  if (msg.type === 'user') {
    ...
    demoTimer = setTimeout(() => {
      isTyping.value = false
      playMessages(msgs, idx + 1)
    }, 1500)
  } else {
    demoTimer = setTimeout(() => playMessages(msgs, idx + 1), 1200)
  }
}
